...
layout = html.Div([
    dbc.Container([
        dbc.Row([
            dbc.Col(html.H1(children='Report Header here'), className="mb-2")
        ]),
        dbc.Row([
            dbc.Col(html.H6(children='Report description'), className="mb-4")
        ]),
...
def update_graph(choice):
    fig = go.Figure(data=[
        #go.Bar(x=df3['continentExp'], y=df3[choice])
